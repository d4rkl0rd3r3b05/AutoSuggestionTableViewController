...
//
//  Created by Mayank Gupta on 20/04/17.
//  Copyright © 2017 tbsl. All rights reserved.
//
...
public class AutoCompleteTextField:UITextField {
    ...
    /// Font for the suggestions type
    public var autoCompleteTypeFont = UIFont(name: "HelveticaNeue-Light", size: 10)
    /// Color of the suggestions type
    public var autoCompleteTypeColor = UIColor.black
    /// Used to set the height of cell for each suggestions
    public var autoCompleteCellHeight:CGFloat = 42.0
    /// Used to set height of keyboard when keyboard is active
    public var keyboardHeight:CGFloat = 54.0
    /// The maximum visible suggestion
    public var maximumAutoCompleteCount = 30
    ...
    public var autoCompleteStrings:[(autoSuggestionText: String, autoSuggestionType: String)]?{
        didSet{ reload() }
    }
    ...
    private func commonInit(){
        ...
        autoCompleteAttributes![NSFontAttributeName] = UIFont(name: "HelveticaNeue-Medium", size: 14)
    ...
    private func setupAutocompleteTable(view:UIView?){
        ...
        let tableView = UITableView(frame: CGRect(x: self.frame.origin.x, y: 60, width: screenSize.width - (self.frame.origin.x * 2), height: screenSize.height - (self.frame.origin.y + self.frame.height) - (self.keyboardHeight + 170)))
        ...
        autoCompleteTableHeight = screenSize.height - (self.frame.origin.y + self.frame.height) - (self.keyboardHeight + 170)
    }
    ...
    private func reload(){
        if enableAttributedText{
            let attrs = [NSForegroundColorAttributeName:autoCompleteTextColor, NSFontAttributeName:autoCompleteTextFont as Any] as [String : Any]
            ...
            if let autoCompleteStrings = autoCompleteStrings, let autoCompleteAttributes = autoCompleteAttributes {
                for i in 0..<autoCompleteStrings.count{
                    let str = autoCompleteStrings[i].autoSuggestionText as NSString
                    let range = str.range(of: text!, options: .caseInsensitive)
                    let attString = NSMutableAttributedString(string: autoCompleteStrings[i].autoSuggestionText, attributes: attrs)
    ...
    func textFieldDidChange(){
        ...
        if text!.isEmpty{ autoCompleteStrings = nil }
         DispatchQueue.main.async { () -> Void in
            self.autoCompleteTableView?.isHidden =  self.hidesWhenEmpty! ? (self.autoCompleteStrings == nil || self.autoCompleteStrings!.count == 0) : false
        }
    }
...
extension AutoCompleteTextField: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if enableAttributedText{
            ...
            cell?.textLabel?.text = autoCompleteStrings![indexPath.row].autoSuggestionText
            
            cell?.detailTextLabel?.font = autoCompleteTypeFont
            cell?.detailTextLabel?.textColor = autoCompleteTypeColor
            
            let suggestionType = autoCompleteStrings![indexPath.row].autoSuggestionType.capitalized
            if suggestionType == "Locations" {
